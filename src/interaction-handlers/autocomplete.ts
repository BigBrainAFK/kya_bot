      ...
      switch (focusedOption.name) {
        ...
        case "value-to-set": {
          ...
          if (BooleanSettings.includes(option)) {
            return this.some(
              ["true", "false"].map((value) => ({ name: value, value }))
            );
          }
          // if option field has todo with numbers show some numbers
          else if (NumberSettings.includes(option)) {
            if (
              isNaN(Number(focusedOption.value)) ||
              !focusedOption.value.length
            ) {
              ...
              return this.some([
                { name: focusedOption.value, value: focusedOption.value },
              ]);
            }
          }
