...
export class InviteListener extends Listener {
  ...
  public async run(message: Message) {
    ...
    if (
      !(await getSettings(message.guild, "antiInvite")) ||
    ...
    if (result.hasLink) {
      message
        .delete()
        .catch(() =>
          message.client.logger.error(`Could not delete invite message.`)
        );
      message.client.emit(
        "caseCreate",
        message,
        ...
        message.client.user,
        message.member!.user,
        `Anti-Invite (${result.link})`,
        "Deleted Message",
        "#ff8300"
      );
      message.author
        .send("Invites are not permitted in this server")
        .catch(() =>
          message.client.logger.error(
            `Could not send DM about invite URLs to \`${message.author.tag}\``
          )
        );
    }
  }
  ...
  public async containsInvite(message: Message): Promise<InviteResult> {
    const discordGGRegex =
      /(?:(?:https?:\/\/)?(?:www\.)?(?:(?:discord|discordapp)\.(?:gg|com)\/(?:invite\/)?).+[a-zA-Z0-9])/gi;
    ...
    if (
      (messageLinks === null || messageLinks.length <= 0) &&
      (messageDiscordGGLinks === null || messageDiscordGGLinks.length <= 0)
    ) {
      return {
        hasLink: false,
        link: null,
    ...
    if (messageDiscordGGLinks.length > 0) {
      for (const link of messageDiscordGGLinks) {
        ...
        const invite = await message.client.fetchInvite(code).catch(() => {});
