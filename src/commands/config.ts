...
export class ConfigCommand extends Command {
  ...
  public override async chatInputRun(
    ...
    if (!interaction.guildId)
      return interaction.editReply(
        "This command is only available inside guilds."
      );
  ...
  async show(interaction: Command.ChatInputInteraction) {
    ...
    if (!option) {
      ...
      for (const currentSetting of AllSettings.sort()) {
        ...
        if (isSettingStringArray(currentSetting)) {
          ...
          interaction.guild?.client.logger.error(
            `There was an error processing ${currentSetting} when building tree`
          );
          continue;
        }
    ...
    // Single setting processing
      ...
      if (isSettingStringArray(option)) {
        ...
        if (!stringArrResult.length)
          return interaction.editReply(`The setting \`${option}\` is empty.`);
  ...
  async set(interaction: Command.ChatInputInteraction) {
    ...
    if (isSettingBoolean(temp)) {
      ...
      if (!data)
        return interaction.editReply(
          `There was an error formatting the data for \`${temp}\`.`
        );
      ...
      return interaction.editReply(
        `The \`set\` function isnt available on \`${option}\``
      );
    }

    if (err && err.length) return interaction.editReply(err);
    return interaction.editReply(
      `Successfully updated \`${option}\` to \`${value}\``
    );
  }
  ...
  async add(interaction: Command.ChatInputInteraction) {
    ...
    if (!option)
      return interaction.editReply("Error: received empty option parameter.");

    if (isSettingStringArray(option)) {
      // check if value is already in the string array. if so error and return
      if (
        (await getSettings(interaction.guild!, option)).some(
          (find) => find === value
        )
      ) {
        return interaction.editReply(
          `\`${value}\` is already set in \`${option}\`.`
        );
      }

      if (value.length > 255)
        return interaction.editReply("The provided value is too long.");
      ...
      if (!data)
        return interaction.editReply(
          `There was an error formatting the data for \`${option}\`.`
        );
      ...
      return interaction.editReply(
        `The \`add\` function isnt available on \`${option}\``
      );
    }
    return interaction.editReply(
      `Successfully added \`${value}\` to \`${option}\`.`
    );
  }
  ...
  async remove(interaction: Command.ChatInputInteraction) {
    ...
    if (isSettingStringArray(option)) {
      ...
      return interaction.editReply(
        `The \`remove\` function isnt available on \`${option}\``
      );
    }
    return interaction.editReply(
      `Successfully removed \`${value}\` to \`${option}\`.`
    );
  }
}
