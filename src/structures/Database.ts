...
class PostgresDatabase {
  ...
  constructor(client: KyaClient, options?: DatabaseOptions) {
    ...
    this._topic = this._dbConn.define("topic", {
      // Per guild settings
  ...
  public async createGuild(guild: Guild): Promise<void> {
    await container.database.guilds
      .create(
        {
          id: guild.id,
  ...
  public async getGuildRow(guild: Guild): Promise<Model<any, any>> {
    const entry = await this._guilds.findByPk(guild.id, {
...
export { DatabaseOptions, PostgresDatabase };
